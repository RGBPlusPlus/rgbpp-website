...
import { ExternalLink, Menu, X, Book, Code, Lock, Shield, Link as LinkIcon, Layers, Network, Server, Zap, Hash, Database, FileText, ChevronDown, ChevronRight, Search } from "lucide-react";
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-6">
                <div className="space-y-3">
                  {/* Documentation with expandable menu */}
                  <div>
                    <button
                      onClick={() => setExpandedSection(expandedSection === -1 ? 0 : -1)}
                      className={cn(
                        "flex items-center justify-between w-full px-4 py-3 rounded-xl text-base font-medium transition-colors duration-200 border border-gray-600/30",
                        pathname.startsWith("/docs")
                          ? "text-white bg-orange-500/20 border-orange-500/30"
                          : "text-gray-300 hover:text-white hover:bg-gray-800/50 hover:border-orange-500/50"
                      )}
                    >
                      <div className="flex items-center space-x-3">
                    ...
                    <div className={cn(
                      ...
                      <div className="mt-3 ml-4 space-y-2">
                        {navigationItems.map((section, sectionIndex) => {
                          const isSubExpanded = expandedSection === sectionIndex;
                          return (
                            <div key={sectionIndex}>
                              <button
                                onClick={() => setExpandedSection(isSubExpanded ? -1 : sectionIndex)}
                                className="flex items-center justify-between w-full px-3 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors duration-200"
                              >
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center space-x-3 px-4 py-3 rounded-xl text-base font-medium text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors duration-200 border border-gray-600/30 hover:border-orange-500/50"
                  >
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center space-x-3 px-4 py-3 rounded-xl text-base font-medium text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors duration-200 border border-gray-600/30 hover:border-blue-500/50"
                  >
